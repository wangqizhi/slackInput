...
pub const STATS: &[(&str, usize)] = &[
    ("e_digimon_level", 0x68),
    ("e_digimon_exp", 0x6c),
    ("e_digimon_current_hp", 0x74),
    ("e_digimon_current_sp", 0x78),
    ("e_digimon_max_hp_bonus", 0xb4),
    ("e_digimon_max_sp_bonus", 0xb8),
    ("e_digimon_atk_bonus", 0xbc),
    ("e_digimon_def_bonus", 0xc0),
    ("e_digimon_int_bonus", 0xc4),
    ("e_digimon_spi_bonus", 0xc8),
    ("e_digimon_spd_bonus", 0xcc),
    ("e_digimon_talent", 0x104),
    ("e_digimon_bond", 0x144),
];
...
impl Group {
    ...
    pub fn variants(self) -> Vec<Variant> {
        match self {
            Self::Battle => vec![Variant {
                pattern: "83 BF * * 00 00 01 0F 85 * * 00 00 * 8D * * * 00 00 E8",
                offset: 0,
                original: "83 BF * * 00 00 01",
            }],
            Self::Cp => vec![
                Variant {
                    pattern: "0F B6 81 A0 01 00 00 88 * 8B 81 A4 01 00 00 89 * 04 8B",
                    offset: 0,
                    original: "0F B6 81 A0 01 00 00",
                },
                Variant {
                    pattern: "41 0F B6 85 A0 01 00 00 88 * * 8B * * * 00 00 89",
                    offset: 0,
                    original: "41 0F B6 85 A0 01 00 00",
                },
            ],
            Self::Scan => vec![
                Variant {
                    pattern: "44 8B 43 08 48 8B CE * * E8 * * * * * 83 * * * * * 75",
                    offset: 0,
                    original: "44 8B 43 08 48 8B CE",
                },
                Variant {
                    pattern: "44 8B 43 08 8B 13 * * * E8 * * * * * 83 * * * * * 75",
                    offset: 0,
                    original: "44 8B 43 08 8B 13",
                },
            ],
            Self::Damage => vec![
                Variant {
                    pattern: "80 7A 24 00 * 8B * * 8B * 0F 85 * * 00 00",
                    offset: 0,
                    original: "80 7A 24 00 * 8B *",
                },
                Variant {
                    pattern: "80 7A 24 00 48 89 D6 49 89 CF 0F 85 * * 00 00",
                    offset: 0,
                    original: "80 7A 24 00 48 89 D6",
                },
            ],
            Self::Items => vec![Variant {
                pattern: "41 83 7F 0C 07 74 * * * * * * * * E8 * * * * 8B 50 0C 2B 50 10 * * * E8 * * * * 41 83 BE E0 00 00 00 04 74",
                offset: 0x13,
                original: "8B 50 0C 2B 50 10",
            }],
            Self::Rewards => vec![
                Variant {
                    pattern: "49 8D 7F 40 44 8B E0 * 8B",
                    offset: 0,
                    original: "49 8D 7F 40 44 8B E0",
                },
                Variant {
                    pattern: "44 8B E8 49 8D 77 40",
                    offset: 0,
                    original: "44 8B E8 49 8D 77 40",
                },
            ],
            Self::Evolution => vec![Variant {
                pattern: "0F 85 * * 00 00 8B 42 04 85 C0 74 * * 88",
                offset: 6,
                original: "8B 42 04 85 C0",
            }],
            Self::Bond => vec![Variant {
                pattern: "0F 57 FF 0F 2E F7 7A * 0F 84 * * 00 00 E8 * * * * * * * * 85 * 75",
                offset: 0,
                original: "0F 57 FF 0F 2E F7",
            }],
            Self::CardRewards => vec![Variant {
                pattern: "39 83 * * 00 00 7E * 89 * * * 00 00 E8 * * * * * * * E8 * * * * * 8D",
                offset: 0,
                original: "39 83 * * 00 00",
            }],
            Self::CardWin => vec![Variant {
                pattern: "48 8B * * * 00 00 E8 * * * * * * * * * * * * * * * * * E8 * * * * 89 83 * * 00 00 E8 * * * * * * * E8 * * * * * 8D * * * 00 00",
                offset: 0x1e,
                original: "89 83 * * 00 00",
            }],
            Self::Stats => vec![Variant {
                pattern: "89 * 24 * E8 * * * * 8B 47 78 BA 01 00 00 00",
                offset: 9,
                original: "8B 47 78 BA 01 00 00 00",
            }],
        }
    }
...
fn assemble(
    ...
    match group {
        Group::Battle => {
            ...
            a.cmp(dword_ptr(rdi + displacement - 4), 1)?;
            a.jne(done)?;
            let mut sp_label = a.create_label();
            a.cmp(dword_ptr(r11), 1)?;
            a.jne(sp_label)?;
            a.mov(dword_ptr(rdi + displacement + 8), 99999)?;
            a.set_label(&mut sp_label)?;
            a.cmp(dword_ptr(r11 + 4), 1)?;
            a.jne(done)?;
            a.mov(dword_ptr(rdi + displacement + 0x10), 99999)?;
        }
        Group::Cp => {
            a.push(rax)?;
            if original[0] == 0x41 {
                a.mov(eax, dword_ptr(r13 + 0x1d0))?;
                a.mov(dword_ptr(r13 + 0x1a4), eax)?;
            } else {
                a.mov(eax, dword_ptr(rcx + 0x1d0))?;
                a.mov(dword_ptr(rcx + 0x1a4), eax)?;
            }
            a.pop(rax)?;
        }
        Group::Scan => {
            a.cmp(dword_ptr(rbx + 8), 0)?;
            a.jle(done)?;
            a.mov(dword_ptr(rbx + 8), 9999)?;
        }
        Group::Damage => {
            let mut enemy = a.create_label();
            a.test(rcx, rcx)?;
            a.je(done)?;
            a.cmp(dword_ptr(rcx + 0xc), 2)?;
            a.je(enemy)?;
            scaled_integer(&mut a, dword_ptr(rdx + 0xc), dword_ptr(r11 + 8), true)?;
            a.jmp(done)?;
            a.set_label(&mut enemy)?;
            scaled_integer(&mut a, dword_ptr(rdx + 0xc), dword_ptr(r11 + 4), false)?;
            a.cmp(dword_ptr(r11), 1)?;
            a.jne(done)?;
            a.mov(dword_ptr(rdx + 0xc), 99999)?;
        }
        Group::Items => {
        ...
        Group::Rewards => {
            ...
            a.cmp(dword_ptr(r11 + 4), 1)?;
            a.jne(done)?;
            a.mov(dword_ptr(r15 + 0x15c), 9999999)?;
        }
        Group::Evolution => {
...
pub fn stat_writes(id: &str, value: i32) -> Result<Vec<(usize, [u8; 4])>, String> {
    ...
    match id {
        "e_digimon_talent" => {
            ...
            Ok(vec![
                (0x104, value.to_le_bytes()),
                (0x108, value.to_le_bytes()),
            ])
        }
        "e_digimon_bond" => {
